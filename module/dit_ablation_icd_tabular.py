...
class ICDTabularDiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, cond_size=512,
        ...
        self.cross_attn_icd = GatedCrossAttention(
            hidden_size, num_heads, icd_embed_dim, cond_size, dropout
        )
    ...
    def forward(self, x, global_cond, icd_embeds, icd_mask=None, return_icd_attn=False):
...
class DiT_ICDTabular_ECG(nn.Module):
    ...
    def __init__(
        ...
        self.cfg_dropout_icd = cfg_dropout_icd
    ...
    def apply_cfg_masks(self, icd_embeds, tab_vector, icd_mask=None):
        
        if not self.training:
            return icd_embeds, tab_vector, icd_mask
        ...
        p_icd = p_tab + self.cfg_dropout_icd                      # [0.20, 0.30)
        ...
        mask_icd[m_tab] = False

        m_icd = (rand >= p_tab) & (rand < p_icd)
        mask_tab[m_icd] = False


        icd_embeds = icd_embeds.clone()
        tab_vector = tab_vector.clone()
        if icd_mask is not None:
            icd_mask = icd_mask.clone()
    ...
    def forward(self, x, t, icd_embeds, age, gender, hr, icd_mask=None,
                return_dict=False, return_icd_attn=False,
                force_uncond_icd=False, force_uncond_tab=False):
        
        B = x.size(0)

        tab_vector = self.tabular_projector(age, gender, hr)

        icd_embeds, tab_vector, icd_mask = self.apply_cfg_masks(
            icd_embeds, tab_vector, icd_mask
        )
        #xiufu
        if not self.training:
        ...
        icd_with_null = torch.cat([icd_embeds, null_token], dim=1)  # [B, N+1, D_icd]
        ...
        for i, block in enumerate(self.blocks):
            ...
            if is_last:
                x, last_icd_attn = block(
                    x, global_cond, icd_with_null, mask_with_null,
                    return_icd_attn=True
                )
            else:
                x = block(x, global_cond, icd_with_null, mask_with_null)
        ...
        if return_dict:
            return {
                "noise_pred": x,
                "icd_weights": last_icd_attn.mean(dim=1).unsqueeze(1)
                    if last_icd_attn is not None else None
            }
        return x
