...
def get_next_experiment_dir(base_dir, exp_type):
    
    os.makedirs(base_dir, exist_ok=True)
    
    existing_dirs = [d for d in os.listdir(base_dir) if d.startswith(f"{exp_type}_")]
    ...
    new_dir = os.path.join(base_dir, f"{exp_type}_{next_num}")
...
@torch.no_grad()
def validate_epoch_dit_dual(dataloader, 
                            dit_model, 
                            diffused_model, 
                            device,
                            ema=None):
    ...
    for data, label in dataloader:
        ...
        icd_embeds = label['icdgraph_embed'].to(device)
        icd_mask = label['icdgraph_mask'].to(device)
...
def train_model_dit_dual(meta, 
    ...
    if is_ddp:
        dit_model = DDP(dit_model, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=False)
    ...
    if resume:
        if not os.path.exists(checkpoint_latest):
            if logger:
                logger.warning(
                    f"Resume mode enabled, but checkpoint_latest.pth not found in: {save_dir}\n"
                    f"   Will start training from scratch in this folder."
                )
            resume_from = None
        else:
            ...
            if logger:
                ...
                logger.info(f"Experiment directory: {save_dir} (#{exp_num})")
    else:
        resume_from = None
        if logger:
            logger.info("=" * 80)
            logger.info("New training mode")
            logger.info(f"New experiment directory: {save_dir} (#{exp_num})")
    
    optimizer = torch.optim.AdamW(
        params=dit_model.parameters(), 
        lr=h_['lr'],
        betas=(0.9, 0.999),
        weight_decay=h_.get('weight_decay', 0.0)
    )
    ...
    if use_ema:
        try:
            ...
            ema = EMA(
                model_without_ddp,
                decay=h_.get('ema_decay', 0.9999),
                update_after_step=h_.get('ema_update_after', 100),
                update_every=h_.get('ema_update_every', 10)
            )
            ema = ema.to(device)
            if logger: logger.info(f"EMA initialized")
        except ImportError as e:
            if logger: logger.warning(f"Could not import EMA module: {e}")
            ema = None
            use_ema = False
    ...
    if resume and resume_from:
        start_epoch, best_val_loss = load_checkpoint(
            resume_from, model_without_ddp, ema, optimizer, scheduler, 
            device, logger
        )
    
    if logger:
        logger.info("=" * 80)
        logger.info(f"Experiment #{exp_num}: {exp_type}")
        logger.info(f"DDP Mode: {is_ddp}")
        if is_ddp: logger.info(f"Local Rank: {local_rank}")
        logger.info(f"Device: {device}")
        logger.info(f"Total epochs: {h_['epochs']} (starting from epoch {start_epoch})")
        logger.info("=" * 80)
    ...
    for i in range(start_epoch, h_['epochs']):
        s_t = time.time()
        ...
        for data, label in dataloader:
            ...
            with torch.cuda.amp.autocast(enabled=True):
                noise_pred = dit_model(
                    x=xt, t=t, icd_embeds=icd_embeds, text_embeds=text_embed,
                    age=age_norm, gender=gender, hr=hr_norm, icd_mask=icd_mask
                )
                loss = F.mse_loss(noise_pred, noise, reduction='mean')
        ...
        val_loss = validate_epoch_dit_dual(
            dataloader=val_dataloader,
            dit_model=model_without_ddp,
        ...
        if logger:
            ema_status = f"| EMA Updates: {ema.num_updates}" if use_ema and ema else ""
            logger.info(f'Epoch: {i+1}/{h_["epochs"]} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | LR: {scheduler.get_last_lr()[0]:.6f} {ema_status}')
        ...
        if not is_ddp or local_rank == 0:
            ...
            save_checkpoint(
                i, model_without_ddp, ema, optimizer, scheduler,
                best_val_loss, save_dir, is_best, logger
            )
            
            save_interval = h_.get('save_interval', 10)
            if (i + 1) % save_interval == 0:
                interval_path = os.path.join(save_dir, f'checkpoint_epoch_{i+1}.pth')
                checkpoint = {
                    'epoch': i,
                ...
                if logger: logger.info(f"Saved interval checkpoint at epoch {i+1}")
        
        e_t = time.time()
        if logger:
            logger.info(f"Epoch Time: {e_t - s_t:.1f}s | Total: {(e_t - start_time)/60:.1f}min")
            logger.info("-" * 80)
    
    if (not is_ddp or local_rank == 0) and ema:
        ema.apply_shadow()
        final_ema_path = os.path.join(save_dir, 'dit_dual_ema_final.pth')
        torch.save(model_without_ddp.state_dict(), final_ema_path)
        if logger: logger.info(f"Saved final EMA weights to {final_ema_path}")
        ema.restore()
