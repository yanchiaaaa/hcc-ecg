...
def validate_epoch(dataloader, dit_model, diffused_model, device, ema=None):
    ...
    for data, label in dataloader:
        ...
        noise_pred = dit_model(
            x=xt, t=t,
            icd_embeds=icd_embeds, icd_mask=icd_mask,
...
def train_model_dit_icd_tabular(meta, save_weights_path, dataloader, val_dataloader,
    ...
    if logger:
        ...
        logger.info(f"{mode} | Ablation-E: ICD+Tabular | {save_dir} (#{exp_num})")
    ...
    if (not is_ddp or local_rank == 0) and ema:
        ema.apply_shadow()
        final_path = os.path.join(save_dir, 'dit_icd_tabular_ema_final.pth')
    ...
    if logger:
        logger.info("=" * 80)
        logger.info("Ablation-E (ICD+Tabular) training completed.")
